...
<head>
    ...
    <body class="mod-bg-1 mod-nav-link">
        <nav class="navbar navbar-expand-lg navbar-dark bg-primary bg-primary-gradient">
            <a class="navbar-brand d-flex align-items-center fw-500" href={{route('index')}}><img alt="logo" class="d-inline-block align-top mr-2" src="img/logo.png"> Учебный проект</a> <button aria-controls="navbarColor02" aria-expanded="false" aria-label="Toggle navigation" class="navbar-toggler" data-target="#navbarColor02" data-toggle="collapse" type="button"><span class="navbar-toggler-icon"></span></button>
        ...
        <main id="js-page-content" role="main" class="page-content mt-3">
            ...
            <div class="row" id="js-contacts">
                @foreach($current_user->getAllUsers() as $user)
                <div class="col-xl-4">
                    <div id="c_{{$user->id}}" class="card border shadow-0 mb-g shadow-sm-hover" data-filter-tags="{{$user->tags}}">
                        <div class="card-body border-faded border-top-0 border-left-0 border-right-0 rounded-top">
                            <div class="d-flex flex-row align-items-center">
                                <span class="status status-{{$user->status}} mr-3">
                                    <span class="rounded-circle profile-image d-block "
                                        style="background-image:url('{{($user->image) ? $user->image : "img/demo/avatars/avatar-m.png" }}'); background-size: cover;">
                                    </span>
                                </span>
                                <div class="info-card-text flex-1">
                                    <a href="javascript:void(0);" class="fs-xl text-truncate text-truncate-lg text-info" data-toggle="dropdown" aria-expanded="false">
                                        {{$user->username}}
                                        @if($current_user->isAdmin() || $current_user->is_equal($user->id)) 
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href={{route('editInfo', $user->id)}}>
                                            <i class="fa fa-edit"></i>
                                        Редактировать</a>
                                        <a class="dropdown-item" href={{route('editSecurity', $user->id)}}>
                                            <i class="fa fa-lock"></i>
                                        Безопасность</a>
                                        <a class="dropdown-item" href={{route('editStatus', $user->id)}}>
                                            <i class="fa fa-sun"></i>
                                        Установить статус</a>
                                        <a class="dropdown-item" href={{route('editPhoto', $user->id)}}>
                                            <i class="fa fa-camera"></i>
                                            Загрузить аватар
                                        </a>
                                        <a href={{route('destroy', $user->id)}} class="dropdown-item" onclick="return confirm('Are you sure?');">
